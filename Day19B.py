...
# unmentioned rule
gen_rule_parse = regex.compile(r'(?P<x>\d+): (?P<y>.*)')
str_rule_parse = regex.compile(r'(?P<x>\d+): (?P<y>"\w")')
...
# modify rules 11 and 8...
# new rules:
# 8: 42 | 42 8
rule8 = str(rules_dict[8])
new8 = rule8
for _ in range(10):
    new8 = r'(' + rule8 + r'|' + rule8 + new8 + r')'
rules_dict[8] = new8

# 11: 42 31 | 42 11 31
rule11 = str(rules_dict[11])
new11 = rule11
rule42 = str(rules_dict[42])
rule31 = str(rules_dict[31])
for _ in range(10):
    new11 = r'(' + rule11 + r'|' + rule42 + new11 + rule31 + r')'
rules_dict[11] = new11
...
print(len(final_rule))

final_regex = regex.compile(final_rule)
