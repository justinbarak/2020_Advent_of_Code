                    ...
                    for y_n in range(y - 1, y + 2):
                        for z_n in range(z - 1, z + 2):
                            if (x_n == x) and (y_n == y) and (z_n == z):
                                continue
                            elif tuple([x_n, y_n, z_n]) in active_points:
                                active_neighbors += 1
                            else:
                                # This means point in surroundings is not an active neighbor
                                continue
                # determine this points state given neighbor activity...
                if (tuple([x, y, z]) in active_points) and (2 <= active_neighbors <= 3):
                    points.add(tuple([x, y, z]))
...
for cyc in range(1, boot_cycles + 1):
    points = cycle(points)
    print_points(points)
    print(f'Total number of active points: {len(points)}')